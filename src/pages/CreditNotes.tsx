...
type CreditNoteStatus = "all" | "draft" | "pending" | "paid" | "overdue";
type CreditNoteDBStatus = "draft" | "pending" | "paid" | "overdue";
type SortField =
  | "invoice_number"
  | "client.name"
  | "issue_date"
...
const CreditNotes = () => {
  ...
  useEffect(() => {
    ...
    if (
      filterParam &&
      ["all", "draft", "pending", "paid", "overdue"].includes(filterParam)
    ) {
                ...
                <Table>
                  ...
                  <TableBody>
                    {paginatedCreditNotes.map((creditNote) => (
                      <TableRow
                        key={creditNote.id}
                        onClick={() => navigate(`/creditnotes/${creditNote.id}`)}
                        className="cursor-pointer"
                      >
